...
for i in range(1,len(x_t[4])):
    xv = (x_t[4][i] - x_t[4][i-1])/timestamp
    x_vel.append(xv)
    yv = (y_t[4][i] - y_t[4][i - 1]) / timestamp
    y_vel.append(yv)
    zv = (z_t[4][i] - z_t[4][i - 1]) / timestamp
    z_vel.append(zv)
    velocity.append(np.sqrt(xv**2 + yv**2 + zv**2))
velocity.append(0)
x_vel.append(0)
y_vel.append(0)
z_vel.append(0)
...
plt.plot(time,velocity, linewidth='1.5',marker='o',markersize='3.5',mec='k')
plt.grid()
plt.xlabel('Time [s]')
...
ax1.plot(time,x_vel, linewidth='1.5',marker='o',markersize='3.5',mec='k')
...
ax2.plot(time,y_vel, linewidth='1.5', color='g',marker='o',markersize='3.5',mec='k')
ax2.grid()
ax2.set_title('Y velocity')
ax2.set_xlabel('Time [s]')
ax2.set_ylabel('Velocity [mm/sec]')

ax3.plot(time,z_vel, linewidth='1.5', color='red',marker='o',markersize='3.5',mec='k')
ax3.grid()
