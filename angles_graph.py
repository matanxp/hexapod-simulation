...
for it in range(len(x_t[0])):
    for leg in [4]:
        angles.append(inverse_kin(x_t[leg][it], y_t[leg][it], z_t[leg][it]))
...
for i in range(len(angles)):
    psi_values.append(angles[i][0])
    theta_values.append(angles[i][1])
    phi_values.append(angles[i][2])

fig,(ax1,ax2,ax3) = plt.subplots(3)

ax1.plot(time,psi_values, linewidth='1.5',marker='o',markersize='3.5',mec='k')
ax1.grid()
ax1.set_title('ψ', size='15')
ax1.set_xlabel('Time [s]')
ax1.set_ylabel('angle [deg]')

ax2.plot(time,theta_values, linewidth='1.5',marker='o',markersize='3.5',mec='k', color='g')
ax2.grid()
ax2.set_title('θ', size = '15')
ax2.set_xlabel('Time [s]')
ax2.set_ylabel('angle [deg]')

ax3.plot(time,phi_values, linewidth='1.5',marker='o',markersize='3.5',mec='k', color='r')
ax3.grid()
ax3.set_title('ϕ', size ='15')
